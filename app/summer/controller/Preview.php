...
//        $kmap_code = $summer_cindex_service->getPreviewKmapCode($topicId);
    ...
    public function studyReport() {
        $request = Request::instance();
        $topicId = $request->param('topicId');
        $module_type = config('preview_class_module_type');
...
//        $scale = round(($total_knowledge_num - $not_learned_weakElements_num) / $total_knowledge_num, 2) * 100;  //知识点掌握率
        $not_learned_weakElements_num = $total_knowledge_num - $has_learned_weakElements_num;

        $scale  = ($has_learned_weakElements_num/$total_knowledge_num)*100;
        $tag_info = array();
        ...
        $bxblStep=$userService->getUserStep($topicId,$user_id,config('preview_class_module_type'));//边学边练
        $bxbl_is_end=0;
...
//        $xiance_module_type = config('l2_xiance_module_type');
        ...
        $tag_name = "";
        $has_answered_questions = $summer_user_service->getUserHasAnsweredQuestionsByModuleType("", $topicId, $module_type);
        $is_enter_Detect = 0;
...
//                "has_answered_questions"=>$has_answered_questions,
            ...
            if (!empty($lastUserLog) && $lastUserLog['is_submit'] != 1) {
                ...
                $tag_code = $summer_user_service->getUserPreviewNextTagCode($kmap_code, $topicId, $used_type,$module_type);
                ...
                if ($tag_code == -1 || $tag_code == "") {
                    ...
                    $question_service = new QuestionService();
                    $question_list = $question_service->getPreviewNextQuestion("", $tag_code, $module_type, $used_type, $topicId);
...
//                        $question_id = "590a9915f4aeb57462445312";
        ...
        $has_learned_tag_code_num = $user_service->getHaveLeardTagCode($user_id, $topicId, $module_type);
        $return_array['has_learned_tag_code_num'] = count($has_learned_tag_code_num);
