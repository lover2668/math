 ...
 */
function SummerAppPreIndex(opts){
    this.opts = opts;
    this.domReady();
}
SummerAppPreIndex.prototype.domReady = function () {
    try{
        var thisObj = this;
        ...
        //$("body").append(MY_UI.htmlspecialcharsDecode("BB&amp;#39;"))
    }catch (err){
        console.log("Error name: " + err.name + "");
        console.log("Error message: " + err.message);
    }finally {}
};
SummerAppPreIndex.prototype.initCharts = function(){
    $("#xx-time-charts").html("");
...
SummerAppPreIndex.prototype.getQuestion = function () {
    try{
        var question;
        var opts = this.opts;
        var topicId = opts.topicId;
        $.ajax({
            url: $url_config.get_pre_exam_questions,
            data: {
                topicId:topicId
            },
            type: 'POST',
...
// 提交试题
SummerAppPreIndex.prototype.initSubmit = function (topicId, answer_content,start_time) {
    try{
        var thisObj = this;
        ...
        $.ajax({
            url: $url_config.submit_pre_exam_question,
            data: {
                topicId: topicId,
                user_id:$_CONFIG.uid,
            ...
            dataType: 'json',
            success: function (response) {
                console.log(typeof response.code != "undefined")
                if(typeof response.code != "undefined"){
                    ...
                    console.log('response.code!="undefined"')
...
//渲染题干信息
SummerAppPreIndex.prototype.createDomTitle = function (opts) {
    try{
        if(opts){
...
SummerAppPreIndex.prototype.createDomContinue = function (times) {
    try {
        var thisObj = this;
        $("[name='xx-continue']").html('<hr/><div name="continue" onclick="_czc.push([&apos;_trackEvent&apos;, &apos;基础先行测试提交点击按钮&apos;, &apos;点击&apos;, &apos;提交&apos;,&apos;5&apos;]);" class="xx-continue-button">提交 </div>');
        ...
        $("[name='continue']").on("click", function () {
            ...
            topicId=parseInt($("input[name=topicId]").val());
            //console.log(type);
...
SummerAppPreIndex.prototype.createDomOption = function (opts) {
    try{
        $("[name='xx-question-option']").html("");
        var optionChild = "";
