 ...
 */
function SummerAppBasicStudy(opts){
    this.opts = opts;
    this.domReady();
}
SummerAppBasicStudy.prototype.domReady = function () {
    try{
        var thisObj = this;
        ...
        if(q_type == 2){
            ...
            //question_title = MY_UI.htmlspecialcharsDecode(MY_UI.rulesFilter(question_title,inputDom));
...
SummerAppBasicStudy.prototype.getQuestion = function () {
    try{
        var question;
        var opts = this.opts;
        var topicId = opts.topicId;
        $.ajax({
            url: $url_config.get_basic_exam_questions,
            data: {
                topicId:topicId
            },
            type: 'POST',
            ...
            success: function (response) {
                if(response.is_end == 0){
                    ...
                    window.open("http://"+window.location.host+"/index.php/"+"/summer/index/baseStudyRedirect/topicId/"+topicId,"_self")
                }
            },
            complete: function () {
            },
...
SummerAppBasicStudy.prototype.initCharts = function(){
    $("#xx-time-charts").html("");
...
// 提交试题
SummerAppBasicStudy.prototype.initSubmit = function (topicId, answer_content,question,start_time) {
    try{
        var thisObj = this;
        ...
        //console.log($_CONFIG);
        $.ajax({
            url: $url_config.submit_base_exam_question,
            data: {
                topicId: topicId,
                user_id:$_CONFIG.uid,
                    ...
                    if (response.isSuccess == 1) {
                        ...
                        $("[name='xx-question-option']").find(".xx-select-box").each(function(index,element){
                            ...
                            if($(this).find("input")[0].checked&&answer_right){
                                ...
                                console.log("!$(this).find()[0].checked")
                                //$(this).addClass("disabled xx-right")
                            }else if($(this).find("input")[0].checked&&!answer_right){
...
SummerAppBasicStudy.prototype.initTagVideo = function(question){

}
SummerAppBasicStudy.prototype.initRedoSubmit = function (topicId, answer_content,question) {
    try{
        var thisObj = this;
        var opts = thisObj.opts;
        var topicId = opts.topicId;
...
SummerAppBasicStudy.prototype.createReDoDom = function(question){
    try{
...
//渲染题干信息
SummerAppBasicStudy.prototype.createDomTitle = function (opts) {
    try{
        if(opts){
            ...
            if(q_type != 2){
                //var question_title = MY_UI.htmlspecialcharsDecode(question_list.content);
...
SummerAppBasicStudy.prototype.createDomContinue = function (opts,times) {
    try{
        var thisObj = this;
        $("[name='xx-continue']").html('<hr/>' +
            ...
            '<div class="prompt prompt-error"><i class="yxiconfont">&#xe646;</i>答错了。再试一次吧<b></b></div><div name="continue" class="xx-continue-button" onclick="_czc.push([&apos;_trackEvent&apos;, &apos;基础学习提交点击按钮&apos;, &apos;点击&apos;, &apos;提交&apos;,&apos;5&apos;]);">提交 </div>')
...
SummerAppBasicStudy.prototype.createRedoDomContinue = function (opts) {
    try{
        var thisObj = this;
        $("[name='xx-continue']").html('<hr/>' +
            '<div class="prompt prompt-wam"><i class="yxiconfont">&#xe646;</i>还是答错了，下一题继续努力吧<b></b></div>' +
            '<div class="prompt prompt-true"><i class="yxiconfont">&#xe647;</i>太棒了！回答正确<b></b></div>' +
            '<div class="prompt prompt-error"><i class="yxiconfont">&#xe646;</i>答错了。再试一次吧<b></b></div><div name="continue" class="xx-continue-button" class="xx-continue-button" onclick="_czc.push([&apos;_trackEvent&apos;, &apos;基础学习第二次提交点击按钮&apos;, &apos;点击&apos;, &apos;提交&apos;,&apos;5&apos;]);">提交 </div>')
        $("[name='continue']").on("click", function () {
            var answer_content = [];
            var answer_val = "",type="",question_id="",answer_url="",topicId="";
            type=$("[name='xx-question-title']").attr("data-q-type");
...
SummerAppBasicStudy.prototype.createDomOption = function (opts) {
    try{
        $("[name='xx-question-option']").html("");
        var optionChild = "";
...
//分析
SummerAppBasicStudy.prototype.questionAnalysis = function(param){
    try {
...
//分步解析
SummerAppBasicStudy.prototype.stepWiseAnalysis = function(param){
    try {
        if(param.question_list.analyze.length==0){
            $("[name='step-wise-analysis']").css("display","none");
        }else{
            ...
            for (var i=0;i<sum_step;i++){
                ...
                stepwiseanalysis += '<li>' +
                    ...
                    '</div>' +
                    '<div class="media-body">' +analyze[i].content+
                    '</div>' +
                    '</div>' +
                    '</li>';
            }
...
SummerAppBasicStudy.prototype.createTimeChart = function(){
    //进入页面时间
