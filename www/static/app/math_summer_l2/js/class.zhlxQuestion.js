...
ZhlxIndex.prototype.getQuestion=function(){
    try{
        ...
        $.ajax({
            url: HOST+"/summer/czhlx/getExamQuestions",
            data:{
...
//                    });
                    ...
                    window.open(HOST+"summer/czhlx/zhlxReport/topicId/"+topicId,"_self");
                    // $(".xx-next").html("结束");
...
ZhlxIndex.prototype.initSubmit=function(topicId,answer_content,start_time){
    try{
        ...
        $.ajax({
            url: HOST+"/summer/czhlx/submitQuestion",
            data:{
                topicId: topicId,
                user_id:$_CONFIG.uid,
...
ZhlxIndex.prototype.initSubmitButton = function () {
    var submitButton = '<div class="modal-container"> <div class="xx-continue" data-flag="0" id="xx-over"> <div class="xx-next js-modal-open" onclick="_czc.push([&apos;_trackEvent&apos;, &apos;竞赛拓展提交点击按钮&apos;, &apos;点击&apos;, &apos;提交&apos;,&apos;5&apos;]);">提交</div> </div> </div>';
    return submitButton;
}
    ...
    try{
        ...
        for(var i = 0;i< optionNum.length;i++){
            optionChild += '<input type="radio" name="rdo" class="rdolist"/>'
                ...
                + '<span class="radiobox-image">'+param.question_list.options[i].key+'、'+MY_UI.htmlDecodeByRegExp(param.question_list.options[i].answer)+'</span>'
                + '</label>';
        }
