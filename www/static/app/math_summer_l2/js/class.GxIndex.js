...
GxIndex.prototype.initSubmit = function (topicId, answer_content, flag, redoHtml, start_time) {
    try {
        ...
        $.ajax({
            url: HOST + 'summer/cbxbl/submitQuestion',
            data: {
                topicId: topicId,
                user_id:$_CONFIG.uid,
                    ...
                    if (response.isSuccess == 1) {
                        if (answer_content[0].is_view_answer == 0 && response.is_right == 0 && flag == 0) {
                            ...
                            if (MY_UI.isEmpty(answer_content[0].answer.replace(/(;)/gi, ""))) {
                                content_html = '<div class="answer-wrong-next"></div>'
                            } else {
                                content_html = '<div class="answer-wrong"></div>'
                            }
                            layer.open({
...
GxIndex.prototype.initRedoSubmit = function (topicId, answer_content, flag, redoHtml) {
    try {
        ...
        var index = layer.load(0, {shade: 0.8});
        $.ajax({
            url: HOST + 'summer/cbxbl/redoSubmitQuestion',
            data: {
                topicId: topicId,
                answer_content: answer_content
            },
...
GxIndex.prototype.getQuestion = function () {
    try {
        ...
        $.ajax({
            url: HOST + "/summer/cbxbl/getExamQuestions",
            data: {
                topicId: topicId,
            },
            type: 'POST',
                ...
                if (response.is_all_end != 1) {
                    if (response.is_enter == 0) {
                        if (!MY_UI.isEmpty(response.question_list)) {
                            ...
                            $("#knowledge-point").html("知识点 " + response.tag_name);
                        ...
                        window.open(HOST + "summer/cbxbl/bTest/topicId/" + topicId, "_self");
                    }
                } else {
                    window.open(HOST + "summer/cbxbl/studyReport/topicId/" + topicId, "_self");
                }
...
GxIndex.prototype.initSubmitButton = function () {
    var submitButton = '<div class="xx-continue" data-flag="0"><div class="xx-continue-inner" onclick="_czc.push([&apos;_trackEvent&apos;, &apos;综合学习提交点击按钮&apos;, &apos;点击&apos;, &apos;提交&apos;,&apos;5&apos;]);">提交</div></div>';
    return submitButton;
}
//初始化填空题
